...
export default function CreatePolicyPage() {
  ...
  const parseAndPopulatePolicyExpression = (expression: string) => {
    ...
    for (const part of parts) {
      if (part.startsWith("(") && part.endsWith(")")) {
        ...
        const groupAttributes = innerContent
          .split(" and ")
          .map((attr) => attr.trim());
    ...
    console.log(
      "🚀 ~ parseAndPopulatePolicyExpression ~ newAttributeGroups:",
      newAttributeGroups
    );
  ...
  const getAllSelectedAttributes = () => {
    return attributeGroups.flatMap((group) =>
      group.attributes.filter((attr) => attr !== "")
    );
  };
                  ...
                  >
                    ...
                    >
                      ...
                      <SelectContent>
                        {availableAttributes
                          .filter((attr) => {
                            const selectedAttrs = getAllSelectedAttributes();
                            return (
